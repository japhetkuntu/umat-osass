...
import { identityClient, nonAcademicClient } from "./apiClient";

class AuthService {
    async login(email: string, password: string): Promise<ApiResponse<LoginResponse>> {
        const result = await identityClient.post<LoginResponse>("/Staffs/login/non-academic", { email, password });
    ...
    async getEligibility(): Promise<ApiResponse<any>> {
        return await nonAcademicClient.get<any>("/Applications/eligibility");
    }
