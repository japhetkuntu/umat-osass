...
const Review = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [stateRes, reviewRes] = await Promise.all([
          ...
          academicService.getOverallReview()
        ]);

        if (stateRes.success) {
          setAcademicState(stateRes.data);
        } else {
  ...
  const reviewData = {
    ...
    publications: {
      level: academicState?.publicationPerformance || "Not Started",
      selfTotal: overallReview?.totalPublicationScore || 0,
      count: academicState?.numberOfRecordsForPublication || 0,
      description: "Curated research output and scholarly publications",
    },
    service: {
      level: academicState?.servicePerformance || "Not Started",
      totalScore: overallReview?.totalServiceScore || 0,
      totalRecords: academicState?.numberOfRecordsForService || 0,
  ...
  const handleSubmit = async () => {
    ...
    if (!academicState?.curriculumVitaeFileName) {
      toast.error("Please upload your Curriculum Vitae before submitting");
      navigate("/application");
      return;
    }
    if (!academicState?.applicationLetterFileName) {
      ...
      <main className="content-container">
        ...
        <div className="space-y-3 mb-10">
          ...
          <SummaryCard
            ...
            title="Teaching"
            icon={GraduationCap}
            level={reviewData.teaching.level}
          >
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground">
                {reviewData.teaching.description}
              </p>
          ...
          <SummaryCard
            ...
            title="Publications"
            icon={BookOpen}
            level={reviewData.publications.level}
          >
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground">
                {reviewData.publications.description}
              </p>
              <div className="flex gap-4 text-sm">
                <span className="text-foreground">
                  {reviewData.publications.count} publications
                </span>
                <span className="text-primary font-medium">
                  Total Score: {reviewData.publications.selfTotal} pts
                </span>
              </div>
            </div>
          </SummaryCard>
