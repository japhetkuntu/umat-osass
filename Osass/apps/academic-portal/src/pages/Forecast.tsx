...
const Forecast = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setIsLoading(true);
        const res = await academicService.getEligibilityForecast();
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 pb-5 border-b border-border">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Track your requirements and timeline for each upcoming academic level.
          </p>
        </div>
          ...
          {forecast.milestones.length === 0 ? (
            <div className="rounded-lg border bg-card p-5">
              <p className="text-sm text-muted-foreground">
                You have reached the highest academic position configured in this system.
              </p>
            </div>
          ) : (
...
const MilestoneCard = ({ milestone, index }: { milestone: PromotionMilestone; index: number }) => {
  ...
  const pubsMet = milestone.hasUploadedData && milestone.publicationsGap === 0;
  const refsMet = milestone.minimumRefereedJournals === 0 || (milestone.hasUploadedData && milestone.refereedJournalsGap === 0);
  ...
  const pubsPct = milestone.minimumPublications > 0
    ? Math.min(100, (milestone.currentPublications / milestone.minimumPublications) * 100)
    : 100;

  const refsPct = milestone.minimumRefereedJournals > 0
    ? Math.min(100, (milestone.currentRefereedJournals / milestone.minimumRefereedJournals) * 100)
    : 100;
    ...
    <div className="rounded-lg border border-border bg-card overflow-hidden">
      ...
      <div className="divide-y divide-border">
        ...
        <div className="px-5 py-5 space-y-3">
          <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-1.5">
            Research Output
            <HelpTip tip="The minimum number of publications and refereed journal articles required for this target rank. Only publications you have uploaded for this specific position are counted here." />
          </p>
          {milestone.hasUploadedData ? (
            <>
              <Req
                met={pubsMet}
                label={`${milestone.minimumPublications} total publication${milestone.minimumPublications !== 1 ? "s" : ""}`}
                status={
                  pubsMet
                    ? `${milestone.currentPublications} publication${milestone.currentPublications !== 1 ? "s" : ""} on record · Requirement met`
                    : `${milestone.currentPublications} of ${milestone.minimumPublications} · ${milestone.publicationsGap} more required`
                }
                pct={pubsPct}
              />
              {milestone.minimumRefereedJournals > 0 && (
                <Req
                  met={refsMet}
                  label={`${milestone.minimumRefereedJournals} refereed journal article${milestone.minimumRefereedJournals !== 1 ? "s" : ""}`}
                  status={
                    refsMet
                      ? `${milestone.currentRefereedJournals} refereed article${milestone.currentRefereedJournals !== 1 ? "s" : ""} on record · Requirement met`
                      : `${milestone.currentRefereedJournals} of ${milestone.minimumRefereedJournals} · ${milestone.refereedJournalsGap} more required`
                  }
                  pct={refsPct}
                />
              )}
            ...
            <div className="rounded border border-dashed border-border px-3 py-3 space-y-1.5">
              <p className="text-[13px] text-muted-foreground">
                No records uploaded for this position yet. Publications and service records you add to your application will be reflected here.
              </p>
              <p className="text-xs text-muted-foreground">
                Required:{" "}
                <span className="font-medium text-foreground/80">
                  {milestone.minimumPublications} publication{milestone.minimumPublications !== 1 ? "s" : ""}
                  {milestone.minimumRefereedJournals > 0
                    ? `, including ${milestone.minimumRefereedJournals} refereed journal article${milestone.minimumRefereedJournals !== 1 ? "s" : ""}`
                    : ""}
                </span>
              </p>
            </div>
        ...
        {milestone.performanceCriteriaOptions.length > 0 && (
          <div className="px-5 py-5 space-y-3">
            <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-1.5">
              ...
              <HelpTip tip="One of these score combinations across your academic activities must be met. Each option shows the minimum required level for each category (e.g. High, Good, Adequate)." />
            </p>
            <p className="text-xs text-muted-foreground">
